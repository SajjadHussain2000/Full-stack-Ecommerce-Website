...
//checking user is logged in or out
window.onload = () => {
    if(user){
        ...
        if(file.type.includes('image')){
            ...
            .then(url => {
                fetch(url, {
                    ...
                    let labil = document.querySelector(`label[for=${fileupload.id}]`);
                    labil.style.backgroundImage = `url(${imageUrl})`;
...
const setFormsData = (data) => {
    ...
    imagePaths.forEach((url,i) => {
        let labil = document.querySelector(`label[for=${uploadImages[i].id}]`);
        labil.style.backgroundImage = `url(${url})`;
        let productImage = document.querySelector('.product-image');
